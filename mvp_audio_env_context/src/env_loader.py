...
def load_environment_daily(config: AudioEnvContextConfig) -> EnvironmentLoadResult:
    ...
    if missing_columns:
        raise ValueError(
            "Environment workbook is missing expected columns: "
...
def _build_env_report(env_df: pd.DataFrame, warnings: list[str]) -> str:
    ...
    missing_counts = env_df.isna().sum().to_dict()
    lines = [
